...
Float32List preprocessImage(CameraImage cameraImage, double modelInputSize) {
  try {
    ...
    final input = Float32List(
      modelInputSize.round() * modelInputSize.round() * 3,
    );
    int bufferIndex = 0;
    ...
    for (int y = 0; y < modelInputSize.round(); y++) {
      for (int x = 0; x < modelInputSize.round(); x++) {
        ...
        if (x < padX ||
          ...
          final srcX = ((x - padX) / scale)
              .clamp(0.0, originalWidth - 1.0)
              .toInt();
          final srcY = ((y - padY) / scale)
              .clamp(0.0, originalHeight - 1.0)
              .toInt();
...
/// 탐지 오류 처리
List<DetectedDefect> handleDetectionError(
  dynamic error,
  int width,
  int height,
) {
